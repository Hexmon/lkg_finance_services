...
const BASE = BASE_URLS.PAYPOINT_BASE_URL; // e.g., 'https://paypoint-uat.bhugtan.in'
const RAW_API_KEY =
    process.env.PAYPOINT_API_KEY ??
    process.env.RETAILER_API_KEY ??
...
function buildUrl(base: string, path: string, query?: Options['query']) {
    // Allow absolute URLs in case a full path is passed
...
/**
 * paypointFetch
 * - Sends X-API-Key when available
 * - Adds JSON Content-Type automatically for non-GET bodies
 * - Parses JSON, but preserves raw text in error for diagnostics
 * - Returns typed T (caller supplies generic)
 */
export async function paypointFetch<T>(path: string, init?: Options) {
    ...
    if (shouldSendJsonBody(method, init?.body)) {
        ...
        // e.g., FormData / Blob / URLSearchParams
        // @ts-expect-error: BodyInit union accepted by fetch
        opts.body = init.body;
    }
    ...
    try {
        ...
        try {
            ...
            // Non-JSON responses are allowed; keep data undefined
        }

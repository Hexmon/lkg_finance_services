...
//     () => billers.find((b) => b.biller_id === billerId || (b as any).billerId === billerId),
//     [billers, billerId]
//   );

//   // read both snake_case and camelCase to be resilient to upstream shape
//   const billerStatus =
//     (selectedBiller as any)?.biller_status ?? (selectedBiller as any)?.billerStatus ?? "INACTIVE";
//   const billerPaymentExactness =
//     (selectedBiller as any)?.biller_payment_exactness ??
//     (selectedBiller as any)?.billerPaymentExactness ??
//     null;
//   const planReqRaw =
//     (selectedBiller as any)?.planMdmRequirement ??
//     (selectedBiller as any)?.plan_mdm_requirement ??
//     "NOT_SUPPORTED";
//   const fetchReqRaw =
//     (selectedBiller as any)?.biller_fetch_requiremet ??
//     (selectedBiller as any)?.billerFetchRequiremet ??
//     "NOT_REQUIRED";
...
//               className={`!h-[45px] !rounded-[12px] !text-white ${
//                 canOpenModal ? "!bg-[#3386FF]" : "!bg-[#3386FF] !cursor-not-allowed !opacity-60"
//               }`}
//               onClick={handleFetch}
//             >
//               Fetch Bill Details
...
export default function BillerPage() {
  ...
  // ── biller list (raw may be unknown or { data: unknown })
  const {
    data: billersRaw = [],
    isError,
    error,
  ...
  const billers: Biller[] = React.useMemo(() => {
  ...
  // read both snake_case and camelCase to be resilient to upstream shape
  const billerStatus =
    (selectedBiller as any)?.biller_status ??
    (selectedBiller as any)?.billerStatus ??
    "INACTIVE";
  const billerPaymentExactness =
    (selectedBiller as any)?.biller_payment_exactness ??
    (selectedBiller as any)?.billerPaymentExactness ??
  ...
  const isActive = billerStatus === "ACTIVE";
  const planReq = (planReqRaw || "NOT_SUPPORTED") as
    | "MANDATORY"
    | "OPTIONAL"
    | "NOT_SUPPORTED";
  const fetchReq = (fetchReqRaw || "NOT_REQUIRED") as
    | "MANDATORY"
    | "OPTIONAL"
    | "NOT_REQUIRED";
  const paymentExactness = (billerPaymentExactness || null) as "EXACT" | "RANGE" | null;

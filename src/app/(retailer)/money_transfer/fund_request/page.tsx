...
export default function FundRequestPage() {
  ...
  // Moved uploadReceiptFile here
  async function uploadReceiptFile(file: File) {
    const url = BASE_URLS.RETAILER_BASE_URL + RETAILER_ENDPOINTS.FUND_REQUEST.RECEIPT_FILE_UPLOAD;
    ...
    try {
      ...
      throw new Error("Network error or CORS issue. Check endpoint and server status.");
    }
  }

  const handleUpload = async (info: any) => {
    if (info.file.status === "uploading") return;
    if (info.file.status === "done" || info.file.status === "removed") return;
    try {
  ...
  const uploadProps = {
    customRequest: async (options: any) => {
      try {
        const result = await uploadReceiptFile(options.file);
  ...
  const handleSubmit = (values: any) => {
    const { receiver, depositDate, paymentMode, amount, ref, remark } = values;
    const payload = {
      requestor_id: "",
      wallet_id: "",
      requestor_type: "RETAILER",
      receipt_url: "",
      reference_id: ref || "",
      payment_receiver_type:
        receiver === "SUPER_ADMIN" ? "DISTRIBUTOR" : "SUPER_DISTRIBUTOR",
      amount: Number(amount),
      remark: remark || "",
      deposit_date: depositDate ? depositDate.format("YYYY-MM-DD") : undefined,
      payment_mode: paymentMode || undefined,
    };

    fetch(BASE_URLS.RETAILER_BASE_URL + RETAILER_ENDPOINTS.FUND_REQUEST.REQUEST_FUND, {
      method: "POST",
      headers: {
  ...
  // Radio button state
  const [receiver, setReceiver] = React.useState("Company");

  return (
    <DashboardLayout
      activePath="/wallet"
      sections={moneyTransferSidebarConfig}
      pageTitle="Fund Request"
    >
            ...
            <Form form={form} layout="vertical" onFinish={handleSubmit}>
              ...
              >
                <div className="flex items-center gap-6">
                  <label className="!text-[14px] !font-medium !whitespace-nowrap">
                    Payment Receiver
                  </label>
                  <Radio.Group
              ...
              <div className="flex items-center justify-center">
                <Button
                  type="primary"
                  htmlType="submit"
                  className="!w-[198px] !h-[40px] !rounded-2xl "
                >
                  Submit
                </Button>
              </div>
          ...
          <Card className="!shadow-md !rounded-2xl !p-6 !space-y-4">
                  ...
                  <>
                    ...
                    >
                      {accounts.map((acc) => {
                        ...
                        const branchName =
                          acc?.branch_name ?? acc?.ifsc_details?.branch ?? "—";
                        const ifsc =
                          acc?.ifsc_code ?? acc?.ifsc_details?.ifsc ?? "—";
                        const maskedAcc = acc?.last4
                          ? `•••• ${acc.last4}`
                          : "—";
                        ...
                        {accounts.map((_, i) => (
                          <button
                            ...
                            className={`h-2 w-2 rounded-full transition-all ${
                              i === current
                                ? "w-4 h-2 rounded-full bg-[#3386FF]"
                                : "bg-[#D9D9D9]"
                            }`}
            ...
            <Card className="!rounded-2xl !shadow-md !col-span-2 !h-[510pxs]">
              ...
              <ul className="!list-disc !ml-6 !space-y-2 !text-[12px] !text-[#7E7A7A] !font-medium">
                <li>
                  Load balance (PG) will update your wallet limit instantly once
                  the transaction is successfull.
                </li>
                <li>
                  The receiver will manually verify the fund request, which may
                  take a few minutes during working days.
                </li>
                <li>For MDR charges kindly check the following details.</li>
                <li>
                  Please do not close the window, refresh the page, press stop
                  or the back button while the transaction is being processed.
                </li>
              </ul>
              ...
              <p className="!text-[12px] !text-[#7E7A7A] !font-medium">
                This service adds money to the wallet via
                NEFT/IMPS/RTGS/UPI/Direct Bank Deposit option and is subject to
                verification by LKG. It may take several minutes.
              </p>
            </Card>
          </Card>
        </div>
